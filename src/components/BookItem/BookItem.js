    ...
    return (
        <Col xs={6} md={4} lg={3} className="mb-5">
            <div className="d-flex justify-content-center mb-24 overflow-lg-auto">
                <img style={{width: "190px"}} className="h-auto" src={props.cover} alt="Book cover" />
            </div>
            <div>
                <p className="fs-14 mb-8 text-grey">{props.category}</p>
                <h3 className="fs-16 mb-8">{props.title}</h3>
                <p className="fs-14 text-grey mb-4">{props.authors}</p>
                <p className="fs-14 mb-3">{props.publishDate}, {props.publishers}</p>
